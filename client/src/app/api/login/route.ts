...
export async function POST(req: NextRequest) {
    ...
    const resUserData = await fetch("http://127.0.0.1:8000/api/customuser/login/", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({phone_number:req_data.phone_number, password:req_data.password}),
    });
    ...
    try {
        ...
        return new NextResponse(JSON.stringify({ message: "Failed to parse user data response", status: 500 }), { status: 500 });
    }

    if (resUserData.ok) {
        console.log(user_data);
        return new NextResponse(JSON.stringify({ message: "success", status: 200, user_data }), { status: 200 });
    } else {
        console.error("Failed to fetch user data");
        return new NextResponse(JSON.stringify({ message: "Failed to fetch user data", status: 500 }), { status: 500 });
    }
}