...
const useTokens = () => {
  ...
  const fetchTokens = useCallback(async (): Promise<void> => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/auth/token/pair`, { // AZAK
        method: "POST",
...
// Only refresh if the refreshToken exists
  const refreshAccessTokens = useCallback(async (): Promise<boolean> => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/auth/token/refresh`, { // AZAK
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const verifyAccessToken = useCallback(async (): Promise<boolean> => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/auth/token/verify`, { // AZAK
        method: "POST",
        headers: { "Content-Type": "application/json" },
