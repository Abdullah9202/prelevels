...
@bundle_router.get("/my-bundles/", response={200: BundleSchema, codes_4xx: dict}, auth=JWTAuth())
async def get_user_bundles(request, *args, **kwargs):
    try:
        ...
        bundles = await sync_to_async(
            lambda: list(user.taking_bundles.all())
        )()
